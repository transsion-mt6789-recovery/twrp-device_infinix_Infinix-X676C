...
void vendor_load_properties()
{
    ...
    for (const string &prop : prop_partitions)
    {
        ...
        property_override(string("ro.product.") + prop + string("name"), "X676C-OP");
        property_override(string("ro.product.") + prop + string("device"), "Infinix-X676C");
        property_override(string("ro.product.") + prop + string("model"), "Infinix X676C");
